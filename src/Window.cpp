...
bool Window::mouse_buttons_[8] = {false};
...
bool Window::mouseButton(GLint button) {
...
void Window::mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    if (core_) {
        core_->mouseButton(button, action);
    }
    if (action == GLFW_PRESS) {
...
void Window::displayFrameRate() {
    s_nbFrames++;
    // Calculate FPS
    if (s_currentTime - s_previousFrameTime >= 1.0) {
